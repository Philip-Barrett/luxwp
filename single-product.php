					...
					<div class="wl-add-cart-row">
						<div class="wl-add-cart-wrap">
							<?php if ( '' !== $membership_download_html ) : ?>
								<div class="wl-membership-download-wrap"><?php echo $membership_download_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
							<?php else : woocommerce_template_single_add_to_cart(); endif; ?>
						</div>
						<?php if ( shortcode_exists( 'yith_wcwl_add_to_wishlist' ) ) : ?>
							<div class="wl-wishlist-wrap"><?php echo do_shortcode( '[yith_wcwl_add_to_wishlist product_id="' . absint( $product_id ) . '"]' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
						<?php endif; ?>
					</div>
					<?php if ( '' === $membership_download_html ) : ?>
					<p class="wl-member-hint"><?php $account_url = function_exists( 'wc_get_page_permalink' ) ? wc_get_page_permalink( 'myaccount' ) : wp_login_url(); printf( wp_kses_post( __( 'Already a member? %s to download.', 'wplend' ) ), '<a href="' . esc_url( $account_url ) . '">' . esc_html__( 'Sign in', 'wplend' ) . '</a>' ); ?></p>
					<?php endif; ?>
